...
class PaperVariantsController extends AppController {
    ...
    public function admin_edit($paper_id) {
        if (!$this->PaperVariant->exists($paper_id)) {
            throw new NotFoundException('Invalid Paper Variant');
        }
        ...
        if ($this->request->is('post') || $this->request->is('put')) {
            ...
            if($this->PaperVariant->save($this->request->data)){
                $this->Session->setFlash('Paper Variant Updated Successfully!!!', 'flash_success');
                $this->redirect('index');
            } else {
                $this->Session->setFlash('Paper Variant Not Updated', 'flash_error');
            }
        } else {
